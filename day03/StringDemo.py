...
# 是否有 shell 這個字
keyword = 'shell'
print(word.find('shell'))  # 會得到 shell 在 word 字串中的位置
if(word.find('shell') >= 0):
    print('{} 中有 shell'.format(word, keyword))
else:
    print('{} 中沒有 shell'.format(word, keyword))
# 是否有 sun 這個字
keyword = 'sun'
print(word.find('sun'))  # 會得到 sun 在 word 字串中的位置, 若得到 -1 表示無此字串
if(word.find('sun') >= 0):
    print('{} 中有 sun'.format(word, keyword))
else:
    print('{} 中沒有 sun'.format(word, keyword))
# 利用 split 來切割字串
